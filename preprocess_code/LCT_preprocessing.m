classdef LCT_preprocessing
   methods (Static)
       function [data_1, data_2, data_3, data_4, data_5] = load_data(filepath)
            % load data
            data_1 = [];
            data_2 = [];
            data_3 = [];
            ...
            for i = 1:length(streams)
                ...
                if info.type == "EEG"
                    ...
                    elseif info.hostname == "DESKTOP-4LI8GO7" && ~isempty(data)
                        data_5 = struct.time_series(7:26, :);
                    end
                end
            end
       end
       %% Extract EEG local time stamps
       function [details] = find_timestamp(filepath)
           % load streams from xdf file
           streams = load_xdf(filepath);
           details = [];
           for i = 1:length(streams)
               s = streams{1, i};
               info = s.info;
               if info.type == "EEG"
                   detail.Desktop = info.hostname;
                   detail.start_time = info.first_timestamp;
                   detail.end_time = info.last_timestamp;
       ...
       function [] = process_eeg(eeg_data, time, player_id)
           ...
           EEG = eeg_checkset( EEG );
           noischa_save = sprintf('/Users/joannaq/Desktop/LIINC/LCT/data/processed_data/try2/noisy_channel/%s_%s.mat', time, player_id);
           ...
           ICA_save_name = sprintf('/Users/joannaq/Desktop/LIINC/LCT/data/processed_data/try2/ica_weights/%s_%s.mat', time, player_id);
           ...
           eeg_save_path = sprintf('/Users/joannaq/Desktop/LIINC/LCT/data/processed_data/try2/processed/%s_%s.mat', time, player_id);
